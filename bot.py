...
class ExcelManager:
    ...
    def _ensure_file_exists(self):
        ...
        try:
            ...
            if not os.path.exists(self.filename):
                wb = Workbook()
                # ⚠️ НЕ удаляем активный лист — иначе файл будет битым!
    ...
    def add_entry(self, user_id: int, time_range: str, description: str, last_name: str = ""):
        try:
            print(f"🔧 Сохраняю запись для user_id: {user_id}")
            wb = openpyxl.load_workbook(self.filename)
            ...
            # Ищем строку за сегодня (начиная со 2-й)
            ...
            if target_row:
                ...
                print(f"🔄 Обновлена запись за {current_date}")
            else:
                ...
                print(f"🆕 Новая запись за {current_date}")
...
# (всё, что ниже — копируется из вашего первого рабочего bot.txt)
...
def main():
    ...
    print("⏱️ Поддержка перезаписи за день")
